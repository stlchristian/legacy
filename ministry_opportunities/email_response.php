...
if($_GET['action']&&$_GET['id']){
	...
	$id=$_GET['id'];
	$action=$_GET['action'];
	if (!file_exists("/mo_temp/$id.php")) {
		print ("This action cannot be completed because a response has already been given.<br>\n");
		mail($errormail,"[Automated] Error on MinOpp Update","Attempt was made to update id[$id] with action [$action] without a /mo_temp file.","From: donotreply@slcconline.edu");
	}
	else{
		if ($action==2){
			$query="DELETE FROM minopps WHERE id='$id' LIMIT 1";
			$cap="Opportunity successfully deleted.";
		}
		elseif ($action==1){
			$query="UPDATE minopps SET post_date=curdate() WHERE id='$id'";
			$cap="Opportunity successfully updated.";
		}
		connect();
		if ($result=mysql_query($query)){
			print $cap."<br>\n";
			unlink ("/mo_temp/$id.php");
		}
		else {
			print "Could not run update. Slcconline staff will be notified.";
			mail($errormail,"[Automated] Error on MinOpp Update","Attempt to update id[$id] with action [$action] failed.","From: donotreply@slcconline.edu");
		}
	}
}
else print "Sorry, this page needs more information."
?>
		<!-- InstanceEndEditable -->
							</td>
						</tr>
